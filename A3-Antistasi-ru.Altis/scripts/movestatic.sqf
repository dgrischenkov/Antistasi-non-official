...
_action = _jugador addAction [localize "STR_DROP_HERE", {{detach _x} forEach attachedObjects player; removeAllActions player},nil,0,false,true,"",""];
...
*/
_cosa addAction [localize "STR_MOVE_THIS_ASSET", "Scripts\moveStatic.sqf",nil,0,false,true,"","(_this == stavros)"];

_cosa setPosATL [getPosATL _cosa select 0,getPosATL _cosa select 1,0];

if (vehicle _jugador != _jugador) exitWith {hint localize "STR_CANT_MOVE_IN_VEH"};

if  (_jugador distance _pos > 30) exitWith {hint localize "STR_CANT_MOVE_30M"};