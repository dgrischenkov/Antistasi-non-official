...
private ["_puntos","_jugador","_puntosJ","_dineroJ", "_incraseCoeff"];
...
if (isMultiplayer) exitWith {
	...
	if (_puntos > 0) then {
		_dineroJ = _dineroJ + (_puntos * _incraseCoeff);
		_jugador setVariable ["dinero",_dineroJ,true];
		_texto = format ["<br/><br/><br/><br/><br/><br/>Money +%1 €",_puntos * _incraseCoeff];
